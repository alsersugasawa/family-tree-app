...
# Family Tree App - Kubernetes Deployment Script
# This script automates the deployment of the Family Tree Application to Kubernetes
...
deploy_application() {
    ...
    kubectl wait --for=condition=ready pod -l app=family-tree-web -n ${NAMESPACE} --timeout=${TIMEOUT} || {
        print_error "Application pods failed to become ready"
        kubectl describe pod -l app=family-tree-web -n ${NAMESPACE}
        exit 1
    }
...
show_access_info() {
    ...
    echo "  kubectl port-forward -n ${NAMESPACE} svc/family-tree-web 8080:80"
    echo "  Then visit: http://localhost:8080"
    echo ""

    # Check if LoadBalancer service exists
    if kubectl get svc family-tree-web -n ${NAMESPACE} -o jsonpath='{.status.loadBalancer.ingress[0].ip}' &> /dev/null; then
        LB_IP=$(kubectl get svc family-tree-web -n ${NAMESPACE} -o jsonpath='{.status.loadBalancer.ingress[0].ip}')
        if [ -n "$LB_IP" ]; then
    ...
    if kubectl get ingress -n ${NAMESPACE} &> /dev/null; then
        INGRESS_HOST=$(kubectl get ingress family-tree-ingress -n ${NAMESPACE} -o jsonpath='{.spec.rules[0].host}' 2>/dev/null)
    ...
    echo "  kubectl logs -f -l app=family-tree-web -n ${NAMESPACE}"
    echo ""
    echo -e "${YELLOW}To scale manually:${NC}"
    echo "  kubectl scale deployment family-tree-web -n ${NAMESPACE} --replicas=5"
}

# Main deployment flow
main() {
    print_header "Family Tree App - Kubernetes Deployment"
